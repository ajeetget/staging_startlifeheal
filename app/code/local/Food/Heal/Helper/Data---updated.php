...
class Food_Heal_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getDishItemsWeight($product,$customDishTotalQty)
	{
	   ...
	   $cooked_item_percent = $product->getCookedItemPercent();
	   $tmpTotalCookedItemWeight = ( $customDishTotalQty * $cooked_item_percent )/100;
	   $totalCookedItemWeight = number_format((float)$tmpTotalCookedItemWeight, 0, '.', '');
	   ...
	   if($baseAssociatedSkus!='')
	   {
	    ...
	    $result.='<table cellpadding="2" cellspacing="2" width="100%"  style="margin-top:0px!important">
				 <tr><td style="width:35%;valign:top"><strong>Total</strong></td>';
		
		$result.='<td>';
		if(isset($cooked_item_percent) && $cooked_item_percent>0)
		{ 
	       $result.='<strong>'.$customDishTotalQty .'( Uncooked)---'.$totalCookedItemWeight.' <br><span style="font-size:13px"> ( Cooked )</span> </strong>';
	    }
		else { $result.='&nbsp;'; }
		$result.='</td></tr>';
    ...
    public function getProductsQtyOfOrderDate($orderdate) {
	...
	public function getProductsQtyBetweenOrderDate($orderdate,$orderdate2) {
		$dbRead  = Mage::getSingleton('core/resource')->getConnection('core_read');	
		
		$selectDatewiseOrders = "SELECT * FROM healorder WHERE orderdate >= '" . $orderdate . "' AND  orderdate <= '" . $orderdate2 . "'";
