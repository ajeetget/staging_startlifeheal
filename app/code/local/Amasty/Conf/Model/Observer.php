...
class Amasty_Conf_Model_Observer 
{
     ...
     * @return $this
     */
    public function updateConfigurablePrice($observer)
    {
        $eventProduct = $observer->getEvent()->getProduct();

        $product = $eventProduct ? $eventProduct : Mage::registry('current_product');

        if (!$product) {
            return $this;
        }

        if ($product->getTypeId() == 'configurable' && Mage::getStoreConfig('amconf/price/display_price')) {
            $priceBlock = Mage::app()->getLayout()->getBlockSingleton('amconf/catalog_product_price');
            if ($priceBlock) {
                $product = $priceBlock->getProduct();
                ...
                $additionalOptions = $observer->getResponseObject()->getAdditionalOptions();
                ...
                $additionalOptions = array_merge(
                    ...
                    array(
                        ...
                        'priceInclTax'    => Mage::helper('core')->currency($priceInclTax, false, false),
                        'priceExclTax'    => Mage::helper('core')->currency($priceExclTax, false, false)
                    )
                );
                $observer->getResponseObject()->setAdditionalOptions($additionalOptions);
            }
        }
