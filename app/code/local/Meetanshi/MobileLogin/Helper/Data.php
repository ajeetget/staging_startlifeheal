...
class Meetanshi_MobileLogin_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    const MSGUSERNAME = 'mobilelogin/apiconfig/uname';
    const MSGPASS = 'mobilelogin/apiconfig/pass';
    ...
    public function Otpsave($data)
    {
        ...
        try {
            if ($otptype == "register") {
                ...
                $collection = Mage::getModel('customer/customer')->getCollection();
                $collection->addAttributeToFilter('primarymobileno', array('eq' => $mobilenumber));
                if (!$collection->getSize()) {
    ...
    public function curlApi($mobilenumber, $message)
    {
        try {
            if ($this->smsEnable()) {
                ...
                } elseif ($this->getApitype() == "msg160") {

                    $msg = urlencode($message);
                    $username = Mage::getStoreConfig(self::MSGUSERNAME);
                    $pass = Mage::getStoreConfig(self::MSGPASS);
                    $senderid = Mage::getStoreConfig(self::SMS_SENDERID);
                    $url = Mage::getStoreConfig(self::SMS_APIURL);

                    $postUrl = $url . "?userid=" . $username . "&pass=" . $pass . "&sender=" . $senderid . "&mobile" . $mobilenumber . "&message=" . $msg . "&route=5";

                    Mage::log($postUrl,null,'mobilelogin.log');
