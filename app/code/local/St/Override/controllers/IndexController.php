...
class St_Override_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function preDispatch(){
        try{        
         $this->con = Mage::helper("heal")->getDbConnection();
        }
        catch(Exception $e) {
            $subject = "Consultation Form Database connection error";
            $message = $e->getMessage(); 
            Mage::helper("heal")->sendEmail($subject,$message); 
        }
        parent::preDispatch();
    }
...
public function getprescriptionAction()  {
    ...
    public function getformrowsAction()  {       
        if (mysqli_connect_errno()) {
            printf("Connect failed: %s\n", mysqli_connect_error());
            exit();
        }
            ...
            if(!empty($_POST['custid'])){
                ...
                $custId = $_POST['custid']; 
                $query = "select * from patientintakeform where custid='".$custId."' and mobileno='".$mobileno."' ORDER BY patientid ASC LIMIT 1";            
                $result   = mysqli_query($this->con, $query); 
                $gender = ''; $age =''; $height_feet = ''; 
                $height_inch = ''; $weight ='';
                $allfields = [];
                 $row = mysqli_fetch_assoc($result);
                    if($row['gender']!=''){
                    ...
                    $allfields =[$gender,$height_feet,$height_inch,$age,$weight];
                    ...
                    foreach($_POST as $key=>$value) {
                        ...
                        if(($_POST[$key]!='')){
                            if($key == 'waist_cm'){
                                ...
                                /*
                                $divider = 12
                                $quotient = $hnum / $divider;
                                $quotient = number_format($quotient);
                                $fields.="height_feet".",";
                                $values.="'".$quotient."'"; 
                                $remainder = $hnum % $divider;
                                $fields.="height_inch";
                                $values.="'".$remainder."'";
                                */
                            }else{
...
public function getconsultationrowsAction()  {
     $config  = Mage::getConfig()->getResourceConnectionConfig("default_setup");
     $dbinfo = array("host" => $config->host,"user" => $config->username,"pass" => $config->password,"dbname" => $config->dbname);    
      $host      = $dbinfo["host"];
      $username  = $dbinfo["user"];
      ...
      if (mysqli_connect_errno()) {
         ...
         exit();
            ...
            $custId = $_POST['custid'];
            $custName = $_POST['custName'];            
            $query = "select * from patientintakeform where custid='".$custId."' and mobileno='".$mobileno."' and consultant_name != 'NULL' ORDER BY date DESC";
            $result   = mysqli_query($con, $query);
            $rowsCount = mysqli_num_rows($result);
            //$result = mysqli_query($con,$query); 
        ...
        while ($row = mysqli_fetch_assoc($result)) {?>
            <tr style="width: auto;margin: 0 10px 10px 0;float:left;"><td  style="cursor: pointer;float: left;list-style: none;display: block;text-align: center;width: auto;border: 1px solid silver;
padding: 10px;margin: 0 10px 10px 0;text-decoration: none;"> 
         <span onclick="showconsultationpdf('<?php echo $row['patientid'];?>','<?php echo $custId;?>','<?php echo $mobileno;?>')">
        <?php echo date('d-M-Y',strtotime($row['date'])) ;?></span>
             </td></tr> 
       <?php }
  ...
  public function getconsultationprintAction()  {
     $config = Mage::getConfig()->getResourceConnectionConfig("default_setup");
     $dbinfo = array("host" => $config->host,"user" => $config->username,"pass" => $config->password,"dbname" => $config->dbname);    
      $host      = $dbinfo["host"];
      $username  = $dbinfo["user"];
      ...
      if (mysqli_connect_errno()) {
         ...
         exit();
            ...
            $alltr ='';
            $allspan ='';          
            $query = "select * from patientintakeform where patientid='".$patientid."' and custid='".$custid."' and mobileno='".$mobileno."'";            
            $result   = mysqli_query($con, $query); 

            $totalfields =['name','date','consultant_name','gender','height_feet','height_inch','age','initial_weight','weight','waist','sleep_quality','mood','energy','stresslevel','sleephour','exercisemin','exerciseday','clinical_diagnosis','current_medication','current_symptoms','recent_success','next_step','other_discussion','follow_up','dietitian_notes'];

            while ($row = mysqli_fetch_assoc($result)) {
            if($row['height_feet']!=''){
                    $heightFeet = $row['height_feet']." Feet";
                   }else{
                    $heightFeet ='';
                   }
                if($row['height_inch']!=''){
                    $heightInch = ", ".$row['height_inch']." Inch";
                    }else{
                       $heightInch = ''; 
                    } 

                if($row['exercisemin']!=''){
                    $Exercisemin = $row['exercisemin']." Minutes a day ";
                   }else{
                    $Exercisemin ='';
                   }
                 if($row['exerciseday']!=''){
                    $Exerciseday = ", ".$row['exerciseday']." Days a week";
                    }else{
                       $Exerciseday = ''; 
                    }
                  if($row['sleephour']!=''){
                    $sleephour = ", ".$row['sleephour']." Hours";
                    }else{
                       $sleephour = ''; 
                    }                
               $alltr.='               
                ...
                <tr>
                   ...
                   <td>'.$row['gender'].'</td>
                   <th>Height:</th>
                   <td>'.$heightFeet.$heightInch.'</td>
                </tr>

                <tr>
                   <th>Waist:</th>    
                   <td>'.$row['waist']." Inch".'</td> 
                   <th>Initial Weight:</th>
                   <td>'.$row['initial_weight']." Kg".'</td>
                   <th>Current Weight:</th>
                   <td>'.$row['weight']." Kg".'</td>
                </tr>
              <tr> <th colspan="6">Clinical Diagnosis:</th>   </tr>
              <tr> <td colspan="6"> '.$row['clinical_diagnosis'].' </td> </tr>
              <tr> <th colspan="6">Current Medication:</th> </tr>
              <tr> <td colspan="6">'.$row['current_medication'].' </td> </tr>
              <tr>  <th colspan="6">Current Symptoms:</th>  </tr>
              <tr>  <td colspan="6">'.$row['current_symptoms'].'</td></tr>
              <tr><th colspan="6">Recent Success:</th> </tr>
              <tr> <td colspan="6">'.$row['recent_success'].'</td> </tr>
              <tr>
                <th>Sleep:</th>    
                <td>'.$sleephour.'</td> 
                <th>Sleep Quality: </th>
              ...
              <tr> <th colspan="6">Other Discussion Notes:</th> </tr>
              <tr> <td colspan="6">'.$row['other_discussion'].'</td> </tr>
              <tr> <th colspan="6">Next Steps:</th>  </tr>
              <tr> <td colspan="6">'.$row['next_step'].' </td> </tr>
              <tr> <th colspan="6">Follow Up:</th>   </tr>
              <tr> <td colspan="6">'.$row['follow_up'].' </td> </tr>

               <tr class="dietitian_notes"> <th colspan="6">Dietitian Notes:</th>   </tr>
              <tr class="dietitian_notes"> <td colspan="6">'.$row['dietitian_notes'].' </td> </tr>

              <tr>
                  <th colspan="2">Consultant Name:</th>
                  <td colspan="4">'.$row['consultant_name'].'</td>
              </tr>
               ';               
            }
            echo $alltr;
