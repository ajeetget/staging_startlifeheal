...
class St_Override_CartController extends Mage_Checkout_CartController
{
	...
	public function addproducttocartAction()
	   { 
				...
				$sendonestepcheckout = Mage::app()->getRequest()->getParam('sendonestepcheckout'); 
				if(is_object($product) && $product->getSku()!='')
				{
					//echo $productId = $product->getEntityId(); 
					$qty       = 1; //Mage::app()->getRequest()->getParam('qty');
					$params    = array( 'product' => $productId,'qty' => $qty );
					...
					// $cart->init();					
					   ...
					   $emailCookieValue = Mage::getModel('core/cookie')->get('myemail');
					   Mage::app()->getFrontController()->getResponse()->setRedirect(Mage::getBaseUrl().'onestepcheckout/');
					   /*
					   if(  ( isset($emailCookieValue) &&   $emailCookieValue!='')  ||  (isset($sendonestepcheckout)) && $sendonestepcheckout=='1' )
					   {
						   Mage::app()->getFrontController()->getResponse()->setRedirect(Mage::getBaseUrl().'onestepcheckout/');
					   }
					   else
					   {
						   $this->_redirectUrl(Mage::getBaseUrl().'email-collect-page');
					   }*/
...
$mobileNoArray = array($mobile);
  ...
  ///////////////////////////End SMS To Customer ////////////////////////
	...
	public function addmultiplecustomoptionsAction()
	{
	            $params  = $this->getRequest()->getParams();
         	           
				$productId = Mage::app()->getRequest()->getParam('id');
				$selectedOptionidOptiontypeId = Mage::app()->getRequest()->getParam('selected');
				
				$qty=1;
				$options = array();
				
				
				$selectedArray = explode(",",$selectedOptionidOptiontypeId);
				foreach($selectedArray as $key=>$bothValues)
				{
					if(isset($bothValues) && $bothValues!='')
					{
						$OptionID_OptionTypeID = explode("_",$bothValues);
						
						$optionid = $OptionID_OptionTypeID[0];
                        $optiontypeid = $OptionID_OptionTypeID[1];
				...
				if(is_object($product) && $productId>0 && $product->getSku()!='')
				{
				  ...
				  $cart->addProduct($product, $params);
				  $needaata = Mage::app()->getRequest()->getParam('needaata');
				  if(isset($needaata) && $needaata=='true')
				  {
					   $aataProductId = 1994;
					   $aataProduct =Mage::getModel('catalog/product')->load($aataProductId);
					   $aataParams = array( 'product' => $aataProductId,'qty' => 1);
					   $cart->addProduct($aataProduct, $aataParams);
				  }
				  $cart->save(); 
				  Mage::getSingleton('checkout/session')->setCartWasUpdated(true); // die;
				  $emailCookieValue = Mage::getModel('core/cookie')->get('myemail');
		  ...
		  else
		  {
				  ...
				  {      
						 ...
						 if( eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$", $email) )
						  { 
											 ////////////////////////////Send Email///////////////////////////////////////
								$to ="diet@lifeheal.in,junior@lifeheal.in,jasmine.chanana@lifeheal.in";
								...
								$bool = mail($to,$subject,$customeMessage,$headers); 
								 if($bool==true ) { echo "Thank you <b>".ucfirst($name)."</b> we will contact you soon"; }
