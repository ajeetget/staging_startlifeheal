        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                try {
					...
					if (!eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $login['username']))
					{
						//echo "mobile no"; //die;
						$customers    = Mage::getResourceModel('customer/customer_collection')
									 ->addAttributeToSelect('*')
									->addAttributeToSelect('primarymobileno')
									->addAttributeToFilter('primarymobileno', $login['username'] )
									->getFirstItem();
							//echo "<pre>";print_r($customers)	;	
										if(isset($customers) && count($customers)==1 && $customers->getEmail()!='')
										{
										   $login['username'] = $customers->getEmail(); //die;
