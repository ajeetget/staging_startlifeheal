...
class St_Override_AccountController extends Mage_Core_Controller_Front_Action
{
	   ...
	   $errUrl = $this->_getUrl('*/*/create', array('_secure' => true));
        ...
        try {
            ...
            if (empty($errors)) {
				...
				$primarymobileno = $post['primarymobileno'];
				$email           = $post['email'];
				$customerName = $firstname." ".$lastname;
				
				$countFirstnameCapitalLetters = preg_match_all("/[A-Z]/", $firstname);
				$countFirstnameLowercaselLetters = preg_match_all("/[a-z]/", $firstname);
					
				$countLasttnameCapitalLetters = preg_match_all("/[A-Z]/", $lastname);
				$countLasttnameLowercaseLetters = preg_match_all("/[a-z]/", $lastname);
					
				$createCustomer = true;
				if( $countFirstnameCapitalLetters>1 && $countFirstnameLowercaselLetters >1)
				{ $createCustomer = false; }
				else if( $countLasttnameCapitalLetters>1 && $countLasttnameLowercaseLetters >1)
				{ $createCustomer = false; }
			
			    if($createCustomer==false)
				{
					$this->_redirectError($errUrl); return;
				}
				...
				////////////////////////////////////////Start entering record in patientintakeform table ///////////////////////////
				...
				/////////////////////////////////////// End entering record in patientintakeform table/////////////////////
				
				
				//////////////////////// inform new customer registeration by sending Email /////////////////////
				...
				// More headers
				$headers .= "From: ".$from . "\r\n";
				$subject = "A New Customer Registered in Lifeheal.in website";
				$message="Hi, a new customer ".$customerName." with mobile no ".$primarymobileno." and email ".$email." registered in website lifeheal.in on  ".date("d-m-Y");
				...
				$address->setCustomerId($customer->getId())
						...
						->setPostcode('11000')
						->setCity('pune')
						->setTelephone($post['primarymobileno'])
						->setFax($post['primarymobileno'])
						//->setCompany('Inchoo')
						->setStreet('Please update Address')
    ...
    protected function _welcomeCustomer(Mage_Customer_Model_Customer $customer, $isJustConfirmed = false)
    {
	   ...
	   $this->_getSession()->addSuccess(
        ...
        $customer->sendNewAccountEmail(
		 ...
		 $tmpCustomer = Mage::getModel('customer/customer')->load($customerId);
         $primaryMobileNo = $customer->getPrimarymobileno();
		 $costomURL = "patientintakeform?mobileno=".$primaryMobileNo; 
		  $costomURL = "welcome"; 
		 $successUrl = rtrim($this->_getUrl($costomURL, array('_secure' => true)), "/");
	...
	public function organhealthAction()
	{
		  $customer = Mage::getSingleton('customer/session')->getCustomer();
		  $firstname = $customer->getFirstname();
		 //echo "<pre>"; print_r($customer);
    ...
    public function prescriptionsAction()
    {
          $customer = Mage::getSingleton('customer/session')->getCustomer();
          $firstname = $customer->getFirstname();
         //echo "<pre>"; print_r($customer);
